...
export class ProjectsComponent implements OnInit {
  ...
  match(term: string,name: string, owner: string,description: string): boolean{
  ...
  search(): void{
    ...
    if(this.searchTerm != ""){
      for(let count  = 0; count < this.data.length; count++){
        if(this.match(this.searchTerm.toLowerCase(),this.data[count].name.toLowerCase(),this.data[count].owner.toLowerCase(),this.data[count].description.toLowerCase())){
  ...
  setOwners(): void{
    for(let count = 0; count < this.data.length; count++){
      this.adminService.getUser(this.data[count].owner).subscribe((result)=>{
        this.data[count].owner = result.name;
      },error=>{
        this.notification = "An error has occurred while retrieving owner of project " + this.data[count].name;
        this.notificationType = 3;
